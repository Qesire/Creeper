...
class StructuredProductionAdapter:
    ...
    *decompressed* byte offsets and keep one decompressor alive across normal
    ...
    def execute_stream(
        ...
        are bound within one durable record. For ordinary line-oriented
        resources, cursor_end is an optional exclusive byte boundary used by
        region harvest. Partial lines at either boundary are discarded.
        """
        if self.kind == "mbox_urls":
