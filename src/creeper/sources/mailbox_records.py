...
Direct evidence requires the message own Date header and URLs extracted only
...
from creeper.sources.non_snapshot import mailbox_year_from_locator
...
def _extract_resource_urls(
    ...
    *,
    ...
    result: list[str] = []
    seen: set[str] = set()
    for match in _RESOURCE_URL_RE.finditer(html.unescape(text)):
    ...
    *,
    ...
    for message_index, match in enumerate(matches):
        ...
        for body in _message_text_parts(message):
            for url in _extract_resource_urls(
                body,
