...
class SourceDiscoveryRegistry:
    ...
    def _ensure_schema(self) -> None:
        self.connection.executescript(
            ...
            CREATE TABLE IF NOT EXISTS source_llm_episodes (
                ...
                trigger_reason TEXT,
                regions_proposed INTEGER NOT NULL DEFAULT 0,
                regions_accepted INTEGER NOT NULL DEFAULT 0,
                deterministic_actions INTEGER NOT NULL DEFAULT 0,
                final_credited_eed REAL NOT NULL DEFAULT 0
            ) WITHOUT ROWID;
            ...
            CREATE INDEX IF NOT EXISTS idx_region_state
                ON source_exploration_regions(state, updated_at);
            CREATE INDEX IF NOT EXISTS idx_region_episode
            ...
            CREATE INDEX IF NOT EXISTS idx_region_edges_source
            ...
            CREATE INDEX IF NOT EXISTS idx_negative_knowledge_scope
                ON source_negative_knowledge(scope_kind, scope_key, reason_code);
        ...
        llm_columns = {str(row[1]) for row in self.connection.execute("PRAGMA table_info(source_llm_episodes)").fetchall()}
        for name, statement in {
            "trigger_reason": "ALTER TABLE source_llm_episodes ADD COLUMN trigger_reason TEXT",
            "regions_proposed": "ALTER TABLE source_llm_episodes ADD COLUMN regions_proposed INTEGER NOT NULL DEFAULT 0",
            "regions_accepted": "ALTER TABLE source_llm_episodes ADD COLUMN regions_accepted INTEGER NOT NULL DEFAULT 0",
            "deterministic_actions": "ALTER TABLE source_llm_episodes ADD COLUMN deterministic_actions INTEGER NOT NULL DEFAULT 0",
            "final_credited_eed": "ALTER TABLE source_llm_episodes ADD COLUMN final_credited_eed REAL NOT NULL DEFAULT 0",
        }.items():
    ...
    def _region_from_row(row: sqlite3.Row) -> ExplorationRegion:
        return ExplorationRegion(
            ...
            created_by_episode_id=row["created_by_episode_id"],
        ...
        with self.connection:
            self.connection.execute(
                ...
                INSERT INTO source_exploration_regions(
                    ...
                    expected_contract_family, context_hash, created_by_episode_id,
                    compiler_version, state, state_reason, created_at, updated_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(region_key) DO NOTHING
                """,
                (
                    region.region_id, region.region_key, region.surface_kind,
                    region.root, region.purpose, region.query_family_json,
                    region.enumerator_spec_json, region.artifact_predicate_json,
                    region.hard_bounds_json, region.stop_conditions_json,
                    region.expected_source_family, region.expected_contract_family,
                    region.context_hash, region.created_by_episode_id,
                    region.compiler_version, region.state.value, region.state_reason,
                    now, now,
                ),
            )
    ...
    def get_region(self, region_id_or_key: str) -> ExplorationRegion | None:
        row = self.connection.execute(
            "SELECT * FROM source_exploration_regions WHERE region_id = ? OR region_key = ?",
            (region_id_or_key, region_id_or_key),
        ).fetchone()
        return None if row is None else self._region_from_row(row)

    def list_regions(self, state: RegionState | str | None = None) -> list[ExplorationRegion]:
        if state is None:
            rows = self.connection.execute(
                "SELECT * FROM source_exploration_regions ORDER BY created_at, region_id"
            ).fetchall()
        else:
            rows = self.connection.execute(
                "SELECT * FROM source_exploration_regions WHERE state = ? ORDER BY created_at, region_id",
                (RegionState(state).value,),
            ).fetchall()
        return [self._region_from_row(row) for row in rows]

    def list_executable_regions(self, *, limit: int = 100) -> list[ExplorationRegion]:
        if limit < 1:
            return []
        rows = self.connection.execute(
            """
            ...
            LIMIT ?
            """,
            (RegionState.READY.value, RegionState.FAILED_RETRYABLE.value, int(limit)),
        ).fetchall()
        return [self._region_from_row(row) for row in rows]

    def transition_region(
        self, region_id: str, new_state: RegionState | str, reason: str = ""
    ) -> ExplorationRegion:
        target = RegionState(new_state)
        self.connection.execute("BEGIN IMMEDIATE")
        try:
            row = self.connection.execute(
                "SELECT * FROM source_exploration_regions WHERE region_id = ?",
                (region_id,),
            ).fetchone()
            if row is None:
            ...
            if current != target and not legal_region_transition(current, target):
                raise StateTransitionError(
                    f"illegal region transition {current.value} -> {target.value}"
                )
            now = float(self.clock())
            self.connection.execute(
                "UPDATE source_exploration_regions SET state = ?, state_reason = ?, updated_at = ? WHERE region_id = ?",
                (target.value, reason, now, region_id),
            )
            self.connection.commit()
        except BaseException:
            self.connection.rollback()
    ...
    def claim_region(self, region_id: str) -> int:
        ...
        try:
            row = self.connection.execute(
                "SELECT state FROM source_exploration_regions WHERE region_id = ?",
                (region_id,),
            ).fetchone()
            if row is None:
                raise KeyError(f"unknown region: {region_id}")
            state = RegionState(str(row["state"]))
            if state not in {RegionState.READY, RegionState.FAILED_RETRYABLE, RegionState.RUNNING}:
                raise StateTransitionError(f"region {region_id} is not claimable from {state.value}")
            progress = self.connection.execute(
                "SELECT execution_generation FROM source_region_progress WHERE region_id = ?",
                (region_id,),
            ).fetchone()
            generation = 1 if progress is None else int(progress["execution_generation"]) + 1
            now = float(self.clock())
            self.connection.execute(
                "UPDATE source_exploration_regions SET state = ?, state_reason = ?, updated_at = ? WHERE region_id = ?",
            ...
            self.connection.execute(
                """
                INSERT INTO source_region_progress(region_id, execution_generation, updated_at)
                VALUES (?, ?, ?)
    ...
    def checkpoint_region(
        self, region_id: str, generation: int, checkpoint: dict[str, object]
    ) -> bool:
        if generation < 1:
    ...
    def finish_region(
        ...
        *,
        ...
        if target not in {RegionState.EXHAUSTED, RegionState.HOLD, RegionState.FAILED_RETRYABLE, RegionState.REJECTED}:
            raise ValueError("finish_region requires a terminal or retryable state")
        self.connection.execute("BEGIN IMMEDIATE")
        try:
            row = self.connection.execute(
                "SELECT state FROM source_exploration_regions WHERE region_id = ?",
            ...
            if row is None or progress is None:
                raise KeyError(f"unknown region or missing progress: {region_id}")
            if int(progress["execution_generation"]) != generation:
                raise RuntimeError("stale or unknown region execution generation")
            current = RegionState(str(row["state"]))
            if not legal_region_transition(current, target):
                raise StateTransitionError(f"illegal region transition {current.value} -> {target.value}")
            now = float(self.clock())
            self.connection.execute(
                "UPDATE source_exploration_regions SET state = ?, state_reason = ?, updated_at = ? WHERE region_id = ?",
                (target.value, reason, now, region_id),
            ...
            if checkpoint is not None:
                self.connection.execute(
                    "UPDATE source_region_progress SET checkpoint_json = ?, updated_at = ? WHERE region_id = ? AND execution_generation = ?",
                    (json.dumps(checkpoint, sort_keys=True, separators=(",", ":")), now, region_id, generation),
                )
            self.connection.commit()
        except BaseException:
            self.connection.rollback()
    ...
    def add_region_source_edge(self, region_id: str, source_key: str, *, relation: str = "produces") -> bool:
        if not relation.strip():
            raise ValueError("region edge relation is required")
        with self.connection:
            changed = self.connection.execute(
                """
                INSERT INTO source_region_edges(region_id, source_key, relation, created_at)
                VALUES (?, ?, ?, ?)
                ON CONFLICT(region_id, source_key, relation) DO NOTHING
                """,
                (region_id, source_key, relation, float(self.clock())),
            ).rowcount
        return changed == 1
    ...
    def record_region_reward(
        ...
        *,
        ...
        values = (scout_proxy_eed, final_accepted_eed, requests, bytes, cost_seconds)
        ...
        with self.connection:
            self.connection.execute(
                ...
                ON CONFLICT(region_id) DO UPDATE SET
                    scout_proxy_eed = excluded.scout_proxy_eed,
                    final_accepted_eed = excluded.final_accepted_eed,
                    requests = excluded.requests,
                    ...
                    updated_at = excluded.updated_at
                """,
                (region_id, float(scout_proxy_eed), float(final_accepted_eed),
                 int(requests), int(bytes), float(cost_seconds), float(self.clock())),
            )

    def upsert_negative_knowledge(self, knowledge: NegativeKnowledge) -> bool:
        with self.connection:
            changed = self.connection.execute(
                """
                ...
                """,
                (
                    f"{knowledge.scope_kind}:{knowledge.scope_key}:{knowledge.reason_code}:{knowledge.policy_version}",
                    knowledge.scope_kind, knowledge.scope_key, knowledge.reason_code,
                    knowledge.evidence_ref, knowledge.policy_version,
                    float(knowledge.created_at or self.clock()), knowledge.expires_at,
                ),
            ).rowcount
        return changed == 1

    def negative_knowledge_matches(
        self, *, scope_kind: str, scope_key: str, reason_code: str | None = None
    ) -> list[NegativeKnowledge]:
        sql = "SELECT * FROM source_negative_knowledge WHERE scope_kind = ? AND scope_key = ?"
        ...
        return [
            NegativeKnowledge(
                scope_kind=str(row["scope_kind"]), scope_key=str(row["scope_key"]),
                reason_code=str(row["reason_code"]), evidence_ref=str(row["evidence_ref"]),
                policy_version=str(row["policy_version"]), created_at=float(row["created_at"]),
    ...
    def prune_expired_negative_knowledge(self) -> int:
        with self.connection:
            return self.connection.execute(
                "DELETE FROM source_negative_knowledge WHERE expires_at IS NOT NULL AND expires_at <= ?",
                (float(self.clock()),),
            ).rowcount
