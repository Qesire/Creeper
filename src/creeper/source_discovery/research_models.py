"""Durable contracts for bounded V7 exploration regions."""
...
_REGION_TRANSITIONS: dict[RegionState, frozenset[RegionState]] = {
    ...
    RegionState.VALIDATED: frozenset({RegionState.READY, RegionState.HOLD, RegionState.REJECTED}),
    RegionState.READY: frozenset({RegionState.RUNNING, RegionState.HOLD, RegionState.REJECTED}),
    RegionState.RUNNING: frozenset({RegionState.EXHAUSTED, RegionState.HOLD, RegionState.FAILED_RETRYABLE}),
    RegionState.FAILED_RETRYABLE: frozenset({RegionState.READY, RegionState.RUNNING, RegionState.REJECTED}),
...
class ExplorationRegion:
    ...
    def __post_init__(self) -> None:
        for name in ("region_id", "surface_kind", "root", "purpose", "expected_source_family", "expected_contract_family", "context_hash", "compiler_version"):
            if not str(getattr(self, name)).strip():
                raise ValueError(f"{name} is required")
        for name in ("query_family_json", "enumerator_spec_json", "artifact_predicate_json", "hard_bounds_json", "stop_conditions_json"):
        ...
        if not key.startswith("region:"):
            raise ValueError("region_key must use region: prefix")
    ...
    def compute_region_key(self) -> str:
        payload = json.dumps(self.semantic_identity(), sort_keys=True, separators=(",", ":"))
