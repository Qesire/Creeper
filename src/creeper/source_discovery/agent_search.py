...
@dataclass(frozen=True)
class ResearchBatch:
    """Proposal-only result of one non-blocking v3 research episode."""
    ...
    contract_proposals: tuple[dict[str, object], ...] = ()
    cost_seconds: float = 0.0


class CommandAgentResearchExecutor:
    """Invoke the v3 child without turning it into a source-search worker.

    This executor intentionally returns proposals only.  Compilation and
    admission remain parent-side operations and no registry mutation occurs.
    """
    ...
    def _request_payload(self, directive: Any, *, episode_id: str) -> dict[str, object]:
        task_type = str(self._directive_value(directive, "task_type", "DISCOVER_NEW_SOURCE"))
        trigger_reason = str(self._directive_value(directive, "trigger_reason", "EXPLICIT_OPERATOR_REQUEST"))
        context = (
            {}
            if self.context_builder is None
            else self.context_builder.build_research_snapshot(directive)
        )
        from creeper.source_discovery.intelligence import SourceIntelligenceContextBuilder
        digest = SourceIntelligenceContextBuilder.digest(context)
        return {
            "contract": "creeper.llm-source-intelligence.v3",
            "prompt_version": "source-intelligence-v3",
            "episode_id": episode_id,
            "execution": {"authority": "proposal_only", "mode": "RESEARCH_COMPILER"},
            "task": {
                "task_type": task_type,
                "trigger_reason": trigger_reason,
                "strategy": str(self._directive_value(directive, "strategy", "META_SOURCE_SEARCH")),
                "subject": self._directive_value(directive, "subject"),
                "desired_regions": int(self._directive_value(directive, "desired_regions", 4)),
                "reason": str(self._directive_value(directive, "reason", "")),
                "objective": "maximize marginal FINAL Accepted Novel EED per total resource cost",
            },
            "context_hash": digest,
            "context": context,
            "region_policy": {
                "min_expected_fanout": 100,
                "max_regions_per_response": 8,
                "max_query_expansion": 512,
                "max_template_expansion": 4096,
            },
        }

    async def __call__(self, directive: Any) -> ResearchBatch:
        invocation_id = uuid.uuid4().hex
        episode_id = f"llm:{invocation_id}"
        ...
        stderr = directory / f"{self.backend}-stderr.log"
        ...
        try:
            ...
            raise SearchAgentProtocolError(f"invalid v3 response JSON: {exc}") from exc
        if not isinstance(data, dict):
            raise SearchAgentProtocolError("v3 response must be an object")
        if set(data) != {"query", "regions", "contract_proposals"}:
            raise SearchAgentProtocolError("v3 response fields are invalid")
        if not isinstance(data["query"], str) or not data["query"].strip():
            raise SearchAgentProtocolError("v3 query must be non-empty")
        if not isinstance(data["regions"], list) or not isinstance(data["contract_proposals"], list):
            raise SearchAgentProtocolError("v3 regions and contract_proposals must be arrays")
        if self.compiler is not None:
            self.compiler.compile_response(data, context_hash=str(payload["context_hash"]), episode_id=episode_id,
                                           task_type=str(payload["task"]["task_type"]))
        return ResearchBatch(
            backend=self.backend, query=data["query"], episode_id=episode_id,
            task_type=str(payload["task"]["task_type"]),
            trigger_reason=str(payload["task"]["trigger_reason"]),
            context_hash=str(payload["context_hash"]), prompt_version="source-intelligence-v3",
            regions=tuple(item for item in data["regions"] if isinstance(item, dict)),
            contract_proposals=tuple(item for item in data["contract_proposals"] if isinstance(item, dict)),
            cost_seconds=max(0.0, float(self.clock()) - started),
        )
