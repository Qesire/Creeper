...
class SourceProducerCliTests(unittest.TestCase):
    ...
    def test_activated_runtime_shrinks_lease_to_backlog_headroom(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with ActivatedSourceRuntime(
                ...
                self.assertAlmostEqual(runtime.producer.range_first_fraction, 0.0)
    ...
    def test_dated_static_runtime_ignores_wayback_headroom(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            task_root = root / "task"
            baseline_root = task_root / "merged260909-3"
    ...
    def test_dated_static_runtime_advances_when_wayback_backlog_is_full(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            task_root = root / "task"
            baseline_root = task_root / "merged260909-3"
            ...
            self.assertEqual(report["rdap_shadow_tasks_enqueued"], 0)
            control = ControlStore(runtime_root / "control.sqlite3")
            try:
    ...
    def test_once_mode_stops_at_durable_evidence_task(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertIsNone(control.get_evidence_task(wayback))
                task = control.get_evidence_task(rdap)
    ...
    def test_undated_static_candidate_pool_keeps_wayback_fallback(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            task_root = root / "task"
            baseline_root = task_root / "merged260909-3"
            ...
            dataset = root / "candidate_pool.txt"
            dataset.write_text("undated.example\n", encoding="utf-8")
            runtime_root = root / "runtime"
            config = root / "undated.toml"
            config.write_text(
                "\n".join(
                    [
                        'source_mode = "static"',
                        ...
                        f'dataset = {json.dumps(str(dataset))}',
                        f'runtime_data_root = {json.dumps(str(runtime_root))}',
                        'source_id = "candidate-pool"',
                        'domain_id = "candidate-pool-domain"',
                        'reservoir_id = "candidate-pool"',
                        "",
                        "[limits]",
                        "queue_source_records = 2",
                        "queue_observations = 2",
            ...
            report = run_once(config, owner="undated-candidate-pool-test")
            ...
            try:
                ...
                self.assertIsNotNone(control.get_evidence_task(wayback))
            finally:
                control.close()
